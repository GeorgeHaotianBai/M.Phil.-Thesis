...
Lemma \autoref{lem:reaction_invertedU} tightens the connection to contest theory in two ways. First, the inverted-U best response is a precise formulation of the ``discouragement'' logic: beyond the peak, additional opposition strength reduces the equilibrium return to Majority enforcement and crowds out effort, as emphasized by \citet{fang2020turning}. Second, the peak $E_m^{\mathrm{peak}}(S_k)$ increases linearly in $S_k$: higher state capacity expands the range of threats for which Majority enforcement responds positively to opposition mobilization. This provides a contest-theoretic mechanism through which a high-$S_k$ incumbent can alleviate the Majority's collective action problem.
